...
#############################################
# Universal Wayland Color Theme Applicator
# Applies .colortheme files system-wide
#############################################
...
log_info "Loading color theme: $(basename "$THEME_FILE")"
...
apply_foot_colors() {
    log_info "Applying colors to foot..."
    ...
    cat > "$foot_colors" << EOF
# Auto-generated by apply-colors
...
apply_btop_colors() {
    log_info "Applying colors to btop..."
    ...
    cat > "$btop_theme" << EOF
# Btop theme auto-generated by apply-colors
# Source: $(basename "$THEME_FILE")
# Generated: $(date)
...
EOF
    ...
    if [ -f "$btop_config" ]; then
        ...
        if grep -q "^color_theme=" "$btop_config"; then
            sed -i 's/^color_theme=.*/color_theme="autotheme"/' "$btop_config"
        else
...
apply_niri_colors() {
    log_info "Applying colors to niri..."
    ...
    if [ -f "$niri_config" ]; then
        remove_auto_section "$niri_config" "// BEGIN AUTO-COLORS" "// END AUTO-COLORS"
    else
        touch "$niri_config"
    fi
...
// BEGIN AUTO-COLORS - Auto-generated, do not edit
// Source: $(basename "$THEME_FILE")
// Generated: $(date)
...
apply_waylock_colors() {
    log_info "Applying colors to waylock..."
...
apply_tuigreet_colors() {
    log_info "✓ tuigreet (inherits foot colors)"
}

apply_mako_colors() {
    log_info "Applying colors to mako..."
    ...
    if [ -f "$mako_config" ]; then
        remove_auto_section "$mako_config" "# BEGIN AUTO-COLORS" "# END AUTO-COLORS"
    else
        touch "$mako_config"
    fi
...
apply_mpv_colors() {
    log_info "Applying colors to mpv..."
    ...
    if [ -f "$mpv_config" ]; then
        remove_auto_section "$mpv_config" "# BEGIN AUTO-COLORS" "# END AUTO-COLORS"
    else
        touch "$mpv_config"
    fi
...
apply_neovim_colors() {
    log_info "Applying colors to neovim..."
...
EOF
    ...
    if [ -f "$nvim_init_lua" ]; then
        if ! grep -q "colorscheme autotheme" "$nvim_init_lua"; then
            ...
            echo "" >> "$nvim_init_lua"
            echo "-- Auto-set colorscheme" >> "$nvim_init_lua"
...
echo ""
log_info "✓ Color theme applied: $(basename "$THEME_FILE")"
log_info "Backups: $BACKUP_DIR"
echo ""
